...
const DEFAULT_PASSWORD = process.env.WALLET_ENCRYPTION_KEY || 'agent-wallet-secure-key-2024';
...
export class AgentWallet {
  ...
  static create(agentName: string, password: string = DEFAULT_PASSWORD): AgentWallet {
    ...
    logger.wallet(`Created wallet for agent "${agentName}" → ${keypair.publicKey.toBase58()}`);
    return new AgentWallet(keypair, id, agentName);
  }

  static load(agentId: string, password: string = DEFAULT_PASSWORD): AgentWallet | null {
    ...
    try {
      const secretKey = decryptPrivateKey(keystore.encryptedPrivateKey, keystore.iv, password);
      const keypair = Keypair.fromSecretKey(secretKey);
      logger.wallet(`Loaded wallet for agent "${keystore.agentName}" → ${keypair.publicKey.toBase58()}`);
  ...
  static fromPrivateKey(privateKeyBase58: string, agentName: string): AgentWallet {
  ...
  async requestAirdrop(solAmount: number = 1): Promise<TransactionSignature> {
    logger.wallet(`Requesting ${solAmount} SOL airdrop for "${this.agentName}"...`);
  ...
  async sendSOL(toAddress: string, amount: number): Promise<TransactionSignature> {
    logger.tx(`${this.agentName} → sending ${amount} SOL to ${toAddress.slice(0, 8)}...`);
  ...
  async sendSPLToken(
    ...
    logger.tx(`${this.agentName} → sending ${amount} SPL tokens (${mintAddress.slice(0, 8)}...) to ${toAddress.slice(0, 8)}...`);
    ...
    const toTokenAccount = await getOrCreateAssociatedTokenAccount(
      this.connection, this.keypair, mint, toPubkey
    );
